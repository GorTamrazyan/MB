...
router.get('/company/:companyId', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const result = await service.getCompanyReviews(req.params.companyId, Number(req.query.page) || 1);
...
router.put('/:id/moderate', authenticate, authorize('ADMIN'), async (req: Request, res: Response, next: NextFunction) => {
  try {
    const data = await service.moderateReview(req.params.id, req.body.isApproved);
