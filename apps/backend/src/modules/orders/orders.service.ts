...
import { CheckoutInput, ItemType } from '@tina/shared';
...
export async function checkout(userId: string, input: CheckoutInput) {
  ...
  if (validatedItems.length === 0) throw new AppError('No valid items in cart', 400);
...
export async function getOrderById(id: string, userId: string) {
  const order = await prisma.order.findUnique({
    where: { id },
    include: { items: { include: { company: { select: { id: true, companyName: true, logoUrl: true } } } } },
  });
...
export async function updateOrderStatus(orderId: string, status: string, companyUserId: string) {
  ...
  if (!hasItem) throw new ForbiddenError('This order does not belong to your company');
...
export async function getCompanyOrders(companyUserId: string, page = 1, pageSize = 10) {
  const company = await prisma.company.findUnique({ where: { userId: companyUserId } });
