...
export async function createPackage(companyUserId: string, data: {
  name: string;
  description?: string;
  discountPercent: number;
  serviceIds: Array<{ serviceId: string; quantity: number; priceOverride?: number }>;
}) {
  ...
  const services = await prisma.service.findMany({
    where: { id: { in: data.serviceIds.map(s => s.serviceId) }, companyId: company.id },
  });

  const totalPrice = services.reduce((sum, s) => {
    const override = data.serviceIds.find(si => si.serviceId === s.id)?.priceOverride;
    const qty = data.serviceIds.find(si => si.serviceId === s.id)?.quantity || 1;
...
export async function getPackage(id: string) {
  return cacheService.getOrSet(`package:${id}`, 600, async () => {
    const pkg = await prisma.package.findUnique({
      where: { id },
      include: { services: { include: { service: { include: { images: { where: { isPrimary: true } } } } } }, company: true },
    });
